...
- (void)setupCancelButton {
    ...
    cancelButton.frame = CGRectMake(0, 0, 150, 52);
    [cancelButton setTitle:NSLocalizedString(@"Cancel", @"Cancel") forState:UIControlStateNormal];

    
    UIEdgeInsets buttonEdge = UIEdgeInsetsMake(5, 10, 5, 15);
    UIImage* normalImage = [UIImage imageNamed:@"button_cancel_normal"];
    UIImage* selectedImage = [UIImage imageNamed:@"button_cancel_pressed"];
    
    normalImage = [normalImage resizableImageWithCapInsets:buttonEdge];
    selectedImage = [selectedImage resizableImageWithCapInsets:buttonEdge];
    
    [cancelButton setBackgroundImage:normalImage forState:UIControlStateNormal];
    [cancelButton setBackgroundImage:selectedImage forState:UIControlStateSelected];
    [cancelButton setBackgroundImage:selectedImage forState:UIControlStateHighlighted];
    [cancelButton setBackgroundImage:selectedImage forState:UIControlStateDisabled];
...
- (void)layoutSubviews {
    ...
    CGRect cancelButtonF = cancelButton.frame;
    if (bounds.size.height < 400) {
      cancelButtonF.origin.y = bounds.size.height - 70;
    } else {
        cancelButtonF.origin.y = bounds.size.height - 120;
    }
    
    cancelButtonF.origin.x = roundf((bounds.size.width - cancelButtonF.size.width) / 2);
